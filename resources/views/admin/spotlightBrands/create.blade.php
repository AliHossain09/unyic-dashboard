<x-app-layout>
    <div class="max-w-3xl mx-auto px-4 py-8">
        <h1 class="text-2xl font-bold mb-6">Add Spotlight Brand</h1>

        <form action="{{ route('spotlight_brands.store') }}" method="POST" enctype="multipart/form-data" class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-6 space-y-6">
            @csrf

            <div>
                <label for="brand" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Brand</label>
                <input list="brand-options" name="brand" id="brand" value="{{ old('brand') }}" class="w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600" placeholder="Select or type a brand">
            ...
            <div>
                ...
                <input type="number" min="1" name="serial" id="serial" value="{{ old('serial') }}" class="w-full px-3 py-2 border rounded-lg dark:bg-gray-700 dark:border-gray-600">
            ...
            <div class="flex justify-end gap-3">
                ...
                <button type="submit" class="px-4 py-2 rounded-lg bg-violet-600 text-white hover:bg-violet-700">Save</button>
            </div>
        </form>
    </div>
</x-app-layout>

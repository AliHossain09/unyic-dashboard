...
<x-app-layout>
    ...
    {{-- <div class="md:mt-6 px-4 sm:px-6 lg:px-8 py-8 w-full max-w-6xl mx-auto dark:text-white mb-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700">
        ...
        <!-- Heading -->
        <h1 class="text-2xl font-bold text-gray-800 dark:text-white mb-6 text-center">Sub Category List</h1>
...
{{-- ...................................................................................................... --}}
  ...
  <div class="md:mt-6 px-4 sm:px-6 lg:px-8 py-8 w-full max-w-7xl mx-auto dark:text-white mb-6 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm">
    ...
    <!-- Heading -->
    <h1 class="text-2xl font-bold text-gray-800 dark:text-white mb-6 text-center border-b border-gray-200 dark:border-gray-700 pb-2">Sub Category List</h1>

    <!-- Filters: one GET form so search + per_page stay in querystring -->
    <form id="filterForm" method="GET" action="{{ route('subCategories.index') }}" class="flex flex-wrap items-center justify-between gap-4 mb-6">
      <!-- Left: Search -->
        ...
        <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
          @forelse ($subCategories as $subCategory)
            <tr>
              <td class="px-4 py-3">
                <div class="flex items-center gap-2">
                  <input type="checkbox" class="form-checkbox rounded text-indigo-600 export-checkbox" value="{{ $subCategory->id }}">
                  <span class="text-sm text-gray-800 dark:text-gray-200">{{ $loop->iteration + (($subCategories->currentPage()-1) * $subCategories->perPage()) }}</span>
                </div>
              </td>
              ...
              <td class="px-4 py-3 text-right">
                <div class="flex items-center justify-end gap-2">
                  <a href="{{ route('subCategories.edit', $subCategory) }}" class="inline-flex items-center p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-700">
                  ...
                  <form action="{{ route('subCategories.destroy', $subCategory) }}" method="POST" onsubmit="return confirmDelete(event)" class="inline-block">
                    @csrf
                    @method('DELETE')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between px-4 py-3 border-t border-gray-200 dark:border-gray-700 mt-4">
      ...
      <div class="mt-3 sm:mt-0">
        @if ($subCategories->lastPage() > 1)
          @php
            $current = $subCategories->currentPage();
            $last = $subCategories->lastPage();
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      exportBtn?.addEventListener('click', () => {
        ...
        if(selected.length === 0){
          Swal.fire('No Selection', 'Please select at least one sub category to export.', 'info');
          return;
        }

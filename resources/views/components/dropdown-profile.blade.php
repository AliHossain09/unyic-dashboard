...
@else
    @if (Route::has('login'))
        <a class="inline-flex justify-center items-center group" href="{{ route('login') }}">
            <img class="w-8 h-8 rounded-full" src="{{ asset('images/user-avatar-32.png') }}" width="32" height="32" alt="Guest" />
            <div class="flex items-center truncate">
                <span class="truncate ml-2 text-sm font-medium text-gray-600 dark:text-gray-100 group-hover:text-gray-800 dark:group-hover:text-white">Sign In</span>
            </div>
        </a>
    @endif
@endif
